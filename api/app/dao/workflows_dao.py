...
from app.schemas.workflow import Definition, WorkflowOut
...
class WorkflowsDao:
    ...
    def get_workflow(self, workflow_id: UUID) -> WorkflowOut | None:
        response = self.client.table("workflows").select("*").eq("id",str(workflow_id)).limit(1).maybe_single().execute()
        return response.data
